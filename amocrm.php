...
define('AC_TLS',false);
define('AC_DB_CS','mysql:host=localhost;port=3306;dbname=asterisk');
define('AC_DB_UNAME','amocrm');
define('AC_DB_UPASS','MvrVjE7N8t7XwEhU');
...
// problems? exiting
if ($resp[0]['response']!=='Success') answer(array('status'=>'error1','data'=>$resp[0]));
...
if ($action==='status'){ // list channels status
	...
	// report error of any
	if ($resp[0]['response']!=='Success') answer(array('status'=>'error2','data'=>$resp[0]));
	...
	file_put_contents('/tmp/amo',strval($_GET['to']) );
	$params=array(
		'action'=>'Originate',
		//'channel'=>'Local/'.intval($_GET['from']).'@amocrm_outgoing',
		'channel'=>'Local/'.strval($_GET['from']).'@amocrm_call_to_manager',
		'Exten'=>strval($_GET['to']),
		'Context'=>'amocrm_outgoing',
		'priority'=>'1',
	...
	//file_put_contents('/tmp/amo',array);
